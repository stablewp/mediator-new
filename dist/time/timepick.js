...
'use strict';
    ...
    this.clearBtn.css({
        color: this.$element.find('input').css('color')
    });
    ...
    this.$element.find('input').on('change', $.proxy(function () {
        this.checkTime();
    },this));
    ...
    this.$element.find('input').click(function(e){
        e.stopPropagation();
        if (!thisRef.tPopup.hasClass('active'))
        {
        	var cur = $(this).val().split(':')
        	$('.picktim-container').hide();
		    $('.picktim-container').removeClass('active');
            ...
            if (thisRef.value() === '')
            {
            	...
            	$('.picktim-hour').val(cur[0]);
            	$('.picktim-mins').val(cur[1]);
            }
...
Plugin.prototype.setOrientation = function(){
    switch (this.settings.orientation)
    {
        ...
        this.tPopup.css({
        top: this.$element.find('input').offset().top + 10 + this.$element.find('input').outerHeight(),
        left: this.$element.find('input').offset().left
        });
        break;
        ...
        this.tPopup.css({
        top: this.$element.find('input').offset().top + 10 + this.$element.find('input').outerHeight(),
        left: this.$element.find('input').offset().left + this.$element.find('input').outerWidth()
        });
        break;
        case this.ORIENTATIONS.topLeft:
        // var pHeight = getComputedStyle(document.body).getPropertyValue("--picktim-container-height");
        this.tPopup.css({
        top: this.$element.find('input').offset().top - 130,
        left: this.$element.find('input').offset().left
        });
        break;
        ...
        this.tPopup.css({
        top: this.$element.find('input').offset().top +  this.$element.find('input').outerHeight(),
        left: this.$element.find('input').offset().left - 10 - this.tPopup.outerWidth()
        });
        break;
        case this.ORIENTATIONS.leftTop:
        this.tPopup.css({
            top: this.$element.find('input').offset().top,
            left: this.$element.find('input').offset().left - 10 - this.tPopup.outerWidth()
        });
        break;
        ...
        this.tPopup.css({
        top: this.$element.find('input').offset().top + this.$element.find('input').outerHeight(),
        left: this.$element.find('input').offset().left + 10 + this.$element.find('input').outerWidth()
        });
        break;
...
Plugin.prototype.updateInput = function(){
    ...
    if (this.settings.mode === this.MODES.h24)
    {
        this.$element.find('input').val(this.tHours.val() + ":" + this.tMins.val());
    }
    else {
        if (this.ampm.text() === 'AM')
        {
            ...
            this.$element.find('input').val((parseInt(this.tHours.val()) + 12) + ":" + this.tMins.val());
        }
...
{
    if (!this.validTime(this.$element.find('input').val()))
    {
        this.$element.find('input').val(this.settings.defaultValue);
    }
    var t = this.$element.find('input').val().split(':');
    ...
    if (this.settings.mode === this.MODES.h24)
    {
        ...
        this.$element.find('input').val(('0' + t[0]).slice(-2) + ':' + ('0' + t[1]).slice(-2));
    }
    else {
