/**
 *  jQuery bkfontIconPicker - v2.0.0
 *
 *  An icon picker built on top of font icons and jQuery
 *
 *  http://codeb.it/bkfontIconPicker
 *
 *  Made by Alessandro Benoit & Swashata
 *  Under MIT License
 *
 * {@link https://github.com/micc83/bkfontIconPicker}
 */
...
;(function ($) {
	...
	var defaults = {
			theme             : 'fip-grey',              // The CSS theme to use with this bkfontIconPicker. You can set different themes on multiple elements on the same page
			...
			iconsPerPage      : 25,                      // Number of icons per page
	...
	function Plugin(element, options) {
		...
		this.iconPicker = $('<div/>', {
			...
			'data-dbicon': $(element).attr('data-icon'),
			'data-dbcolor': $(element).attr('data-color'),
			'data-dbhighlight': $(element).attr('data-highlight'),
		...
		$('.icons-selector').each(function(){
			var color = $(this).parent().find('select option[value="'+$(this).attr('data-dbicon')+'"]').attr('data-color');
			
			var highlight = $(this).parent().find('select option[value="'+$(this).attr('data-dbicon')+'"]').attr('data-highlight')
			...
			inst.setSelectedIcon($(this).attr('data-dbicon'),color,highlight);
		});
	...
	Plugin.prototype = {
		...
		init: function () {
			...
			// If current element is SELECT populate settings.source
				...
				this.element.find('option').each($.proxy(function (i, el) {
					...
					newIconColor = $(el).attr('data-color') ? $(el).attr('data-color') : false,
					newIconHighlight = $(el).attr('data-highlight') ? $(el).attr('data-highlight') : false;
					if (newIconValue) {
						this.settings.source.push(newIconValue);
						this.settings.color.push(newIconColor);
						this.settings.highlight.push(newIconHighlight);
			...
			this.iconContainer.on('click', '.fip-box', $.proxy(function (e) {
				this.setSelectedIcon($(e.currentTarget).find('i').attr('data-fip-value'),$(e.currentTarget).attr('data-color'),$(e.currentTarget).attr('data-highlight'));
				this.toggleIconSelector();
			}, this));
		...
		renderIconContainer: function () {
			...
			for (var i = 0, item; item = iconsPaged[i++];) {
				...
				var color = this.settings.color[i-1],
					highlight = this.settings.highlight[i-1]
				// Set the icon box
				$('<span/>', {
					html:'<i data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + parseInt(item, 10).toString(16) + ';' : item ) + '"') : 'class="' + item + '"') + '></i><div class="half-panel" style="background-color:'+highlight+'"></div>',
					'class':'fip-box',
		 ...
		 */
		setSelectedIcon: function (theIcon,color,highlight) {	

			this.iconPicker.find('.selected-icon').removeClass('icon-today icon-full icon-half icon-pending icon-pending-half')

			if(color != 'false'){
				this.iconPicker.find('.selected-icon').css('background-color',color);
				this.iconPicker.find('.selected-icon').addClass(theIcon);
			}else {
				this.iconPicker.find('.selected-icon').css('background-color','transparent');
			}
			...
			if(highlight){
				this.iconPicker.find('.selected-icon i').css('background-color',highlight);
			}else {
				this.iconPicker.find('.selected-icon i').css('background-color','transparent');
			}
	...
	$.fn.bkfontIconPicker = function (options) {

		// Instantiate the plugin
		this.each(function () {
			if (!$.data(this, "bkfontIconPicker")) {
				$.data(this, "bkfontIconPicker", new Plugin(this, options));
			}
		});
		...
		this.setIcons = $.proxy(function (newIcons, iconSearch) {
			...
			this.each(function () {
				$.data(this, "bkfontIconPicker").settings.source = newIcons;
				$.data(this, "bkfontIconPicker").settings.searchSource = iconSearch;
				$.data(this, "bkfontIconPicker").initSourceIndex();
				$.data(this, "bkfontIconPicker").resetSearch();
				$.data(this, "bkfontIconPicker").loadIcons();
			});
		}, this);
		...
		this.destroyPicker = $.proxy(function() {
			this.each(function() {
				...
				$.data(this, "bkfontIconPicker").iconPicker.remove();
				// Reset the CSS
				$.data(this, "bkfontIconPicker").element.css({
					visibility: '',
					top: '',
					position: '',
					zIndex: '',
				...
				// destroy data
				$.removeData(this, "bkfontIconPicker");
			});
		}, this);
		...
		this.refreshPicker = $.proxy(function(newOptions) {
			...
			this.each(function() {
				if (!$.data(this, "bkfontIconPicker")) {
					$.data(this, "bkfontIconPicker", new Plugin(this, newOptions));
				}
			});
		}, this);
