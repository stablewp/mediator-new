	    ...
	    function column_title($item){
	    	...
	    	$type = get_post_type($item['ID']);
	  		if($type == 'med-booking'){
		        ...
		        $rowtitle = '<strong><a href="#" class="row-title bk-srcdelete" data-day="'.$fields['booking_day'].'">'.$item['title'].'<span style="color:#c03">'.$past.'</span></a></strong>';
	    	...
	    	$type = get_post_type($item['ID']);
	  		if($type == 'med-booking'){
		    	...
		    	if(strrpos($fields['booked'], 'true') !== false){
			        ...
			        $bactions = $item['status'] == 'pending' ? '<div class="med-booking-actions"><p class="bk-approve" data-url="'.plugins_url().'/'.MYPLUGIN_PLUGIN_NAME.'" data-id="'.$item['ID'].'">Accept</p><p class="bk-deny">Reject</p></div>': '<div class="med-booking-actions"><p class="bk-cancel">Cancel</p></div>';
			        return sprintf('%1$s',
